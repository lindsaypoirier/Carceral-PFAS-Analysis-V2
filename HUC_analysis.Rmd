...
ggplot(aes(x = DATA_SOURCE, fill = VALUE)) +
  ...
  theme(legend.title = element_text(size = 8), 
        legend.text = element_text(size = 8)) + 
  scale_fill_manual(values=c("#ececec", "#00bfc4")) +
  geom_text(data = summary_colocation_counts_source %>% filter(VALUE == "Yes"), 
            aes(label = paste0(round(..count../nrow(pb_huc_elevation) * 100, 
                                     digits = 1), "%")), 
            stat = "count", 
            colour = "black", 
...
c <- summary_colocation_counts_source %>%
  ...
  ggplot(aes(x = DATA_SOURCE, fill = JUVENILE)) +
  geom_bar(position = "dodge") +
  coord_flip() +
  theme_minimal()  +
...
rm(susp_colocated_facilities_source, known_colocated_facilities_source, a, b, c)
```
